...
// --- HELPER COMPONENTS (DEFINED FIRST TO AVOID REFERENCE ERRORS) ---
...
function ChatOverlay({ t, theme, messages, onClose, onSend }) {
    ...
    return (
        <div className={`absolute inset-0 z-50 flex flex-col animate-slide-up ${theme.bg}`}>
...
function SettingsView({ t, theme, isDarkMode, language, setLanguage, openInfo, currency, setCurrency, toggleTheme }) {
  ...
  return (
    <div className="animate-fade-in space-y-4 pb-12 w-full">
      <div className="flex items-center justify-between mb-2"><h2 className={`text-2xl font-bold ${theme.text}`}>{t.settings_title}</h2><span className={`text-[10px] px-2 py-1 rounded border ${isDarkMode ? 'bg-lime-400/10 text-lime-400 border-lime-400/20' : 'bg-emerald-100 text-emerald-700 border-emerald-200'}`}>v2.6.1</span></div>
  ...
  return (
    <div className={`min-h-screen ${theme.bg} ${theme.text} font-sans selection:bg-lime-400 selection:text-black flex justify-center overflow-hidden transition-colors duration-500`}>
      <div className={`w-full max-w-md ${theme.bg} min-h-screen relative flex flex-col shadow-2xl shadow-lime-900/20 border-x ${theme.border} transition-colors duration-500`}>
        ...
        <button onClick={() => setIsChatOpen(true)} className={`absolute bottom-28 right-6 w-14 h-14 ${theme.accentBg} rounded-full shadow-[0_4px_20px_rgba(163,230,53,0.4)] flex items-center justify-center text-black z-50 hover:scale-110 active:scale-90 transition-transform cursor-pointer`}>
