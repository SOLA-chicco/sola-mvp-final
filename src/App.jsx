...
const TRANSLATIONS = {
  ...
  jp: {
    // JP omitted for brevity, fallback to English/ID in logic if needed, but structure kept.
    mode_business: "利益モード",
    mode_home: "夢モード",
...
const App = () => {
  ...
  // --- THEME COLOR SYSTEM (THE SOURCE OF TRUTH) ---
  const theme = {
    ...
    // Dynamic Accent: Neon Lime for Dark Mode, Darker Emerald/Lime for Light Mode readability
  ...
  return (
    <div className={`min-h-screen ${theme.bg} ${theme.text} font-sans selection:bg-lime-400 selection:text-black flex justify-center overflow-hidden transition-colors duration-500`}>
     ...
     <div className="w-full md:max-w-md bg-zinc-950 min-h-screen relative flex flex-col md:shadow-2xl md:shadow-lime-900/20 md:border-x border-zinc-800">
        ...
        <header className="p-6 pb-2 relative z-10">
          ...
          <div className="flex justify-between items-start mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                onClick={() => setSystemHealth(prev => prev === 'ok' ? 'warning' : 'ok')} 
                className={`flex items-center gap-2 px-3 py-1.5 rounded-full border transition-all ${systemHealth === 'ok' ? (isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-zinc-100 border-zinc-200') : 'bg-yellow-500/10 border-yellow-500/30'}`}
              >
                 {systemHealth === 'ok' ? (
                   <CheckCircle size={16} className="text-emerald-500" />
                 ) : (
                   <Wrench size={16} className="text-yellow-500 animate-pulse" />
                 )}
              </button>
              ...
              <button 
                ...
                className={`w-8 h-8 rounded-full border flex items-center justify-center hover:${theme.accentText} transition-colors relative ${isDarkMode ? 'bg-zinc-800 border-zinc-700' : 'bg-zinc-100 border-zinc-200'}`}
              >
              ...
              <button 
                ...
                className={`w-8 h-8 rounded-full border flex items-center justify-center hover:${theme.accentText} transition-colors ${isDarkMode ? 'bg-zinc-800 border-zinc-700' : 'bg-zinc-100 border-zinc-200'}`}
              >
          ...
          {systemHealth === 'warning' && (
             <div className="mb-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-3 flex items-center justify-between animate-fade-in-down">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold text-yellow-500">{t.sys_health_warn}</p>
                    <p className={`text-[10px] ${theme.textSec}`}>{t.sys_health_msg}</p>
                  </div>
                </div>
                <button 
                  onClick={handleFixMaintenance}
                  className="bg-yellow-500 text-black text-[10px] font-bold px-3 py-1.5 rounded-md hover:bg-yellow-400 transition-colors shadow-lg shadow-yellow-500/20"
                >
                  {t.sys_btn_fix}
                </button>
             </div>
          ...
          {activeTab === 'dashboard' && (
            <div className={`p-1 rounded-xl flex relative border animate-fade-in-down ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-zinc-100 border-zinc-200'}`}>
              <div 
                className={`absolute top-1 bottom-1 w-[calc(50%-4px)] ${theme.accentBg} rounded-lg transition-all duration-300 ease-out shadow-[0_0_10px_rgba(163,230,53,0.3)] ${mode === 'home' ? 'translate-x-[100%] ml-1' : 'translate-x-0'}`}
              ></div>
              <button onClick={() => setMode('business')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg z-10 transition-colors ${mode === 'business' ? 'text-black font-bold' : `${theme.textSec} hover:${theme.text}`}`}>
                <Briefcase size={18} />
                <span className="text-sm">{t.mode_business}</span>
              </button>
              <button onClick={() => setMode('home')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg z-10 transition-colors ${mode === 'home' ? 'text-black font-bold' : `${theme.textSec} hover:${theme.text}`}`}>
                <Home size={18} />
                <span className="text-sm">{t.mode_home}</span>
              </button>
            </div>
          )}
        </header>
        ...
        <main className="flex-1 overflow-y-auto p-6 pt-2 pb-24 scrollbar-hide">
          ...
          {activeTab === 'dashboard' && mode === 'business' && (
            <div className="mb-4 animate-fade-in relative z-20">
              ...
              >
                <span className={`text-sm font-bold group-hover:${theme.accentText} transition-colors ${theme.text}`}>{currentDevice}</span>
                <ChevronDown size={14} className={`${theme.textSec} transition-transform ${isDeviceMenuOpen ? 'rotate-180' : ''}`} />
              </button>
              {isDeviceMenuOpen && (
                <div className={`absolute top-8 left-0 w-56 border rounded-xl shadow-xl overflow-hidden animate-fade-in-down ${theme.card}`}>
                  <div className="p-1">
                    {["Kopi Kenangan Cabang 7", "Cabang 8 (Surabaya)", "Cabang 9 (Bandung)"].map((dev, idx) => (
                      <button key={idx} onClick={() => handleDeviceChange(dev)} className={`w-full text-left px-3 py-2 text-xs rounded-lg flex items-center justify-between ${currentDevice === dev ? `${isDarkMode ? 'bg-lime-400/10 text-lime-400' : 'bg-emerald-50 text-emerald-600'}` : `${theme.textSec} hover:${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-100'} hover:${theme.text}`}`}>
                        {dev}
                        {currentDevice === dev && <Check size={12} />}
                      </button>
                    ))}
                    <div className={`h-[1px] ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-200'} my-1`}></div>
                    <button className={`w-full text-left px-3 py-2 text-xs ${theme.accentText} hover:${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-100'} rounded-lg font-medium`}>{t.device_add}</button>
                  </div>
                </div>
              )}
            </div>
          ...
          {activeTab === 'dashboard' && mode === 'business' && (
            <BusinessView 
              t={t} theme={theme} isDarkMode={isDarkMode}
              productionVol={productionVol} 
              setProductionVol={setProductionVol}
              openInfo={(key) => setActiveInfo(key)}
              onExport={() => showToast(t.b2b_export_success)}
            />
          )}
          {activeTab === 'dashboard' && mode === 'home' && (
            <HomeView 
              t={t} theme={theme} isDarkMode={isDarkMode}
              dreamGoal={dreamGoal} 
              onEditGoal={() => setIsEditingDream(true)}
              onShare={() => showToast(t.b2c_share_success)}
            />
          )}
          {activeTab === 'energy' && (
            <EnergyView t={t} theme={theme} isDarkMode={isDarkMode} openInfo={(key) => setActiveInfo(key)} />
          )}
          {activeTab === 'settings' && (
            <SettingsView 
              t={t} theme={theme} isDarkMode={isDarkMode}
        ...
        {/* Floating Consultation Button (FAB) */}
        <button 
          onClick={() => setIsChatOpen(true)}
          className={`absolute bottom-24 right-6 w-14 h-14 ${theme.accentBg} rounded-full shadow-[0_4px_20px_rgba(163,230,53,0.4)] flex items-center justify-center text-black z-40 hover:scale-110 transition-transform`}
        >
        ...
        {/* Bottom Nav */}
        <div className={`absolute bottom-0 w-full backdrop-blur-md border-t py-4 px-6 flex justify-between items-center z-30 ${isDarkMode ? 'bg-zinc-950/90 border-zinc-800' : 'bg-white/90 border-zinc-200'}`}>
        ...
        {/* --- TOAST & MODALS --- */}
        {toastMessage && (
           <div className="absolute bottom-24 left-1/2 -translate-x-1/2 bg-zinc-800 text-white px-4 py-2 rounded-full shadow-lg border border-zinc-700 flex items-center gap-2 animate-slide-up z-50 whitespace-nowrap">
...
// --- ENTERPRISE SETTINGS VIEW (FIXED) ---
const SettingsView = ({ t, theme, isDarkMode, language, setLanguage, openInfo, currency, setCurrency, toggleTheme }) => {
  ...
  const SectionHeader = ({ id, icon, title, subtitle }) => (
    <button 
      ...
      className={`w-full flex items-center justify-between p-4 border ${activeSection === id ? `rounded-t-2xl border-b-0 ${isDarkMode ? 'bg-zinc-800/50' : 'bg-zinc-100'} ${theme.border}` : `rounded-2xl ${isDarkMode ? 'bg-zinc-900' : 'bg-white'} ${theme.border} hover:opacity-80`} transition-all`}
    >
      <div className="flex items-center gap-3">
         <div className={`p-2 rounded-lg ${activeSection === id ? `${theme.accentBg} text-black` : `${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-100'} ${theme.textSec}`}`}>
           {React.cloneElement(icon, { size: 18 })}
         </div>
         <div className="text-left">
           <h4 className={`text-sm font-bold ${activeSection === id ? theme.text : theme.textSec}`}>{title}</h4>
           {subtitle && <p className={`text-[10px] ${theme.textSec}`}>{subtitle}</p>}
         </div>
      </div>
      <ChevronUp className={`${theme.textSec} transition-transform ${activeSection === id ? '' : 'rotate-180'}`} size={16} />
    </button>
  );

  const containerClass = `${theme.card} border-t-0 rounded-b-2xl p-4 space-y-4 animate-slide-down`;
  ...
  return (
    <div className="animate-fade-in space-y-4 pb-12">
      ...
      <div className="space-y-0">
        <SectionHeader id="general" icon={<Settings />} title={t.settings_cat_general} subtitle={t.settings_cat_general_sub} />
        {activeSection === 'general' && (
          <div className={containerClass}>
             <div className="space-y-1">
               <label className={`text-xs ${theme.textSec}`}>{t.settings_lang}</label>
               <div className="flex gap-2">
                 {['id', 'en', 'jp'].map(l => (
                   <button key={l} onClick={()=>setLanguage(l)} className={`flex-1 py-2 text-xs rounded border ${language===l ? `${theme.accentBg} text-black font-bold border-transparent` : `${theme.inputBg} ${theme.textSec} ${theme.inputBorder}`}`}>
                     {l.toUpperCase()}
                   </button>
                 ))}
               </div>
             ...
             <div className={itemClass}>
                <span className={`text-xs ${theme.textSec}`}>{t.settings_currency}</span>
                <select 
                   value={currency} 
                   onChange={(e) => setCurrency(e.target.value)}
                   className={`${theme.inputBg} ${theme.inputBorder} border rounded p-1 text-xs ${theme.accentText} focus:outline-none`}
                >
             ...
             <button onClick={toggleTheme} className={`w-full ${itemClass}`}>
                <span className={`text-xs ${theme.textSec}`}>{t.settings_dark_mode}</span>
                <div className={`w-8 h-4 rounded-full relative transition-colors ${isDarkMode ? 'bg-lime-400' : 'bg-zinc-300'}`}>
                  <div className={`absolute top-0.5 w-3 h-3 bg-black rounded-full transition-all ${isDarkMode ? 'right-0.5' : 'left-0.5'}`}></div>
                </div>
             </button>
          </div>
        )}
      ...
      {/* ENERGY */}
      <div className="space-y-0">
        <SectionHeader id="energy" icon={<Zap />} title={t.settings_cat_energy} subtitle={t.settings_cat_energy_sub} />
        {activeSection === 'energy' && (
          <div className={containerClass}>
             <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className={`text-[10px] ${theme.textSec} flex items-center gap-1`}>
                    {t.settings_tariff}
                    <button onClick={() => openInfo('tariff')} className="hover:text-current"><HelpCircle size={10} /></button>
                  </label>
                  <input type="number" defaultValue="1444" className={`w-full ${theme.inputBg} ${theme.inputBorder} border rounded p-2 text-xs ${theme.inputText}`} />
                </div>
                <div className="space-y-1">
                   <label className={`text-[10px] ${theme.textSec} flex items-center gap-1`}>
                    {t.settings_grid_limit}
                    <button onClick={() => openInfo('grid_limit')} className="hover:text-current"><HelpCircle size={10} /></button>
                   </label>
                   <input type="number" defaultValue="6600" className={`w-full ${theme.inputBg} ${theme.inputBorder} border rounded p-2 text-xs ${theme.inputText}`} />
                </div>
             </div>
             
             <div className={`space-y-2 pt-2 border-t ${theme.border}`}>
                <div className="flex justify-between text-xs">
                   <span className={`${theme.textSec} flex items-center gap-1`}>
                     {t.settings_batt_threshold}
                     <button onClick={() => openInfo('batt_threshold')} className="hover:text-current"><HelpCircle size={10} /></button>
                   </span>
                   <span className={theme.accentText}>{batteryThreshold}%</span>
                </div>
                <input 
                  type="range" min="0" max="100" value={batteryThreshold} onChange={e=>setBatteryThreshold(e.target.value)}
                  className={`w-full h-1.5 rounded-lg appearance-none cursor-pointer ${isDarkMode ? 'bg-zinc-800 accent-lime-400' : 'bg-zinc-200 accent-emerald-600'}`}
                />
                <p className={`text-[10px] ${theme.textSec}`}>{t.settings_batt_desc}</p>
             </div>
             ...
             <div className={itemClass}>
                <div>
                   <p className={`text-xs ${theme.text} flex items-center gap-1`}>
                     {t.settings_export}
                     <button onClick={() => openInfo('export')} className={`${theme.textSec} hover:text-current`}><HelpCircle size={10} /></button>
                   </p>
                   <p className={`text-[10px] ${theme.textSec}`}>{t.settings_export_desc}</p>
                </div>
                <div className={`w-8 h-4 rounded-full relative ${isDarkMode ? 'bg-zinc-700' : 'bg-zinc-300'}`}><div className={`absolute left-0.5 top-0.5 w-3 h-3 bg-white rounded-full`}></div></div>
             </div>
          </div>
        )}
      </div>
      ...
      {/* NOTIFICATIONS */}
      <div className="space-y-0">
        <SectionHeader id="notif" icon={<Bell />} title={t.settings_cat_notif} subtitle={t.settings_cat_notif_sub} />
        {activeSection === 'notif' && (
          <div className={containerClass}>
             <div className={`p-3 rounded-lg border space-y-2 ${isDarkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-zinc-50 border-zinc-200'}`}>
               <label className={`text-xs ${theme.textSec} flex items-center gap-1`}>
                 {t.settings_custom_rule}
                 <button onClick={() => openInfo('alert_rule')} className="hover:text-current"><HelpCircle size={10} /></button>
               </label>
               <div className="flex gap-2">
                  <select className={`${theme.inputBg} text-xs ${theme.inputText} p-2 rounded border ${theme.inputBorder}`}><option>Battery Level</option><option>Grid Load</option></select>
                  <select className={`${theme.inputBg} text-xs ${theme.inputText} p-2 rounded border ${theme.inputBorder}`}><option>&lt;</option><option>&gt;</option></select>
                  <input className={`w-12 ${theme.inputBg} text-xs ${theme.inputText} p-2 rounded border ${theme.inputBorder}`} defaultValue="10" />
                  <button className={`${theme.accentBg} text-black text-xs px-3 rounded font-bold`}>Add</button>
               </div>
             </div>
             
             <div className="space-y-2">
                <p className={`text-xs ${theme.textSec}`}>{t.settings_channels}</p>
                {['Push Notifications', 'Email Digest', 'WhatsApp (Premium)'].map(c => (
                   <label key={c} className={`flex items-center gap-2 p-2 rounded cursor-pointer ${isDarkMode ? 'hover:bg-zinc-950' : 'hover:bg-zinc-100'}`}>
                      <div className={`w-4 h-4 rounded border flex items-center justify-center ${theme.accentBg} border-transparent`}><Check size={10} className="text-black"/></div>
                      <span className={`text-xs ${theme.textSec}`}>{c}</span>
                   </label>
                ))}
             </div>
          </div>
      ...
      {/* DEVELOPER */}
      <div className="space-y-0">
        <SectionHeader id="dev" icon={<Code />} title={t.settings_cat_dev} subtitle={t.settings_cat_dev_sub} />
        {activeSection === 'dev' && (
          <div className={containerClass}>
             <div className="space-y-1">
                <label className={`text-[10px] ${theme.textSec} flex items-center gap-1`}>
                  {t.settings_api_key}
                  <button onClick={() => openInfo('api')} className="hover:text-current"><HelpCircle size={10} /></button>
                </label>
                <div className="flex gap-2">
                   <div className={`flex-1 ${theme.inputBg} border ${theme.inputBorder} rounded p-2 flex items-center justify-between`}>
                      <code className={`text-[10px] ${theme.accentText} font-mono`}>{apiKeyVisible ? 'sk_live_51Mz...9sT' : '•••••••••••••••••••••'}</code>
                      <button onClick={()=>setApiKeyVisible(!apiKeyVisible)} className={`${theme.textSec} hover:${theme.text}`}>
                         {apiKeyVisible ? <EyeOff size={12}/> : <Eye size={12}/>}
                      </button>
                   </div>
                   <button className={`${isDarkMode ? 'bg-zinc-800 hover:bg-zinc-700' : 'bg-zinc-200 hover:bg-zinc-300'} px-3 rounded border ${theme.inputBorder}`}><Copy size={14} className={theme.textSec}/></button>
                </div>
             </div>
             <div className="space-y-1">
                <label className={`text-[10px] ${theme.textSec} flex items-center gap-1`}>
                  {t.settings_webhook}
                  <button onClick={() => openInfo('webhook')} className="hover:text-current"><HelpCircle size={10} /></button>
                </label>
                <input defaultValue="https://api.pos-system.com/sola-hook" className={`w-full ${theme.inputBg} border ${theme.inputBorder} rounded p-2 text-xs ${theme.textSec} font-mono`} />
             </div>
          </div>
        )}
      </div>
...
// --- ENTERPRISE ACCOUNT CENTER (PROFILE) ---
const ProfileOverlay = ({ t, theme, isDarkMode, onClose, openInfo }) => {
  const [activeTab, setActiveTab] = useState('team'); 

  return (
    <div className={`absolute inset-0 ${isDarkMode ? 'bg-black/90' : 'bg-zinc-900/50'} backdrop-blur-md z-50 flex items-center justify-center p-4 animate-fade-in`} onClick={onClose}>
      <div className={`${theme.card} w-full h-[85vh] rounded-3xl relative flex flex-col overflow-hidden shadow-2xl`} onClick={e=>e.stopPropagation()}>
        ...
        <div className={`p-6 border-b ${theme.border} flex justify-between items-start ${isDarkMode ? 'bg-zinc-900' : 'bg-white'}`}>
           <div className="flex items-center gap-4">
              <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${isDarkMode ? 'from-lime-400 to-emerald-600' : 'from-emerald-400 to-teal-500'} p-[2px]`}>
                 <div className={`w-full h-full ${isDarkMode ? 'bg-zinc-900' : 'bg-white'} rounded-[14px] flex items-center justify-center`}>
                    <User size={32} className={theme.text}/>
                 </div>
              </div>
              <div>
                 <h2 className={`text-xl font-bold ${theme.text}`}>Chicco</h2>
                 <p className={`${theme.accentText} text-xs font-medium uppercase tracking-wider mb-1`}>Owner • Kopi Kenangan 7</p>
                 <span className={`text-[10px] ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-100'} ${theme.textSec} px-2 py-0.5 rounded border ${theme.border}`}>ID: #SOLA-8821</span>
              </div>
           </div>
        ...
        {/* Tabs */}
        <div className={`flex border-b ${theme.border} ${isDarkMode ? 'bg-zinc-950/50' : 'bg-zinc-50'}`}>
           {[
             {id: 'team', label: t.profile_team, icon: Users},
             {id: 'asset', label: t.profile_assets, icon: Server},
             {id: 'billing', label: t.profile_billing, icon: CreditCard}
           ].map(tab => (
              <button 
                key={tab.id}
                onClick={()=>setActiveTab(tab.id)}
                className={`flex-1 py-4 flex flex-col items-center gap-1 border-b-2 transition-colors ${activeTab===tab.id ? `border-${isDarkMode ? 'lime-400' : 'emerald-600'} ${theme.accentText} ${isDarkMode ? 'bg-lime-400/5' : 'bg-emerald-50'}` : 'border-transparent text-zinc-400 hover:text-zinc-500'}`}
              >
                 <tab.icon size={18} />
        ...
        <div className={`flex-1 overflow-y-auto p-6 ${isDarkMode ? 'bg-zinc-950/30' : 'bg-zinc-50'}`}>
           ...
           {activeTab === 'team' && (
             <div className="space-y-6 animate-fade-in">
                <div className="flex justify-between items-center">
                   <h3 className={`font-bold text-sm ${theme.text}`}>{t.profile_team}</h3>
                   <button className={`text-[10px] flex items-center gap-1 ${theme.accentBg} text-black px-2 py-1 rounded font-bold hover:opacity-80`}><PlusCircle size={12}/> {t.profile_invite}</button>
                </div>
                <div className="space-y-3">
                   {[
                     {name: 'Chicco', role: 'Owner', email: 'owner@kopikenangan.com', status: 'Active'},
                     {name: 'Budi Santoso', role: 'Manager', email: 'budi.mgr@kopikenangan.com', status: 'Active'},
                     {name: 'Siti Aminah', role: 'Technician', email: 'tech.support@vendor.com', status: 'External'},
                   ].map((user, i) => (
                     <div key={i} className={`flex items-center justify-between p-3 border ${theme.border} rounded-xl ${isDarkMode ? 'bg-zinc-900' : 'bg-white shadow-sm'}`}>
                        <div className="flex items-center gap-3">
                           <div className={`w-8 h-8 rounded-full ${isDarkMode ? 'bg-zinc-800 text-zinc-400' : 'bg-zinc-100 text-zinc-600'} flex items-center justify-center text-xs font-bold`}>{user.name.charAt(0)}</div>
                           <div>
                              <p className={`text-xs font-bold ${theme.text}`}>{user.name}</p>
                              <p className={`text-[10px] ${theme.textSec}`}>{user.email}</p>
                           </div>
                        </div>
                        <div className="text-right">
                           <span className={`text-[10px] px-2 py-0.5 rounded-full ${user.role==='Owner' ? `${isDarkMode ? 'bg-lime-400/20 text-lime-400' : 'bg-emerald-100 text-emerald-700'}` : `${isDarkMode ? 'bg-zinc-800 text-zinc-400' : 'bg-zinc-100 text-zinc-600'}`}`}>{user.role}</span>
                        </div>
                     </div>
                   ))}
                </div>
                ...
                <div className={`pt-4 border-t ${theme.border} space-y-3`}>
                   <h3 className={`font-bold text-sm ${theme.text}`}>{t.profile_business_info}</h3>
                   <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1"><label className={`text-[10px] ${theme.textSec}`}>NPWP</label><input className={`w-full ${theme.inputBg} border ${theme.inputBorder} rounded p-2 text-xs ${theme.inputText}`} defaultValue="82.111.222.3-444.000"/></div>
                      <div className="space-y-1">
                        <label className={`text-[10px] ${theme.textSec} flex items-center gap-1`}>
                          Site ID
                          <button onClick={() => openInfo('site_id')} className="hover:text-current"><HelpCircle size={10} /></button>
                        </label>
                        <input className={`w-full ${theme.inputBg} border ${theme.inputBorder} rounded p-2 text-xs ${theme.inputText}`} defaultValue="JKT-SEL-007"/>
                      </div>
                   </div>
                </div>
             </div>
           ...
           {activeTab === 'asset' && (
             <div className="space-y-6 animate-fade-in">
                <div className={`border ${theme.border} rounded-xl p-4 flex items-center gap-4 ${isDarkMode ? 'bg-zinc-900' : 'bg-white shadow-sm'}`}>
                   <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-100'}`}><Server className={theme.accentText} /></div>
                   <div className="flex-1">
                      <h4 className={`text-sm font-bold ${theme.text}`}>Sola Inverter X-2000</h4>
                      <p className={`text-[10px] ${theme.textSec}`}>SN: 9982-1123-AABB</p>
                      <div className={`mt-2 w-full h-1.5 rounded-full overflow-hidden ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-200'}`}>
                         <div className={`w-[60%] h-full rounded-full ${theme.accentBg}`}></div>
                      </div>
                      <div className={`flex justify-between mt-1 text-[9px] ${theme.textSec}`}>
                         <span>{t.profile_warranty}: 4 Yrs</span>
                         <span>Active</span>
                      </div>
                   </div>
                </div>
                ...
                <div className="space-y-3">
                   <h3 className={`text-xs font-bold uppercase tracking-wider ${theme.textSec}`}>{t.profile_maintenance}</h3>
                   <div className={`flex gap-3 items-start relative pb-4 border-l ${theme.border} ml-2 pl-4`}>
                      <div className={`absolute -left-[5px] top-0 w-2.5 h-2.5 rounded-full ${theme.accentBg}`}></div>
                      <div>
                         <p className={`text-xs font-bold ${theme.text}`}>Routine Cleaning</p>
                         <p className={`text-[10px] ${theme.textSec}`}>20 Oct 2025 • by Tech Siti</p>
                      </div>
                   </div>
                   <div className={`flex gap-3 items-start relative pb-4 border-l ${theme.border} ml-2 pl-4`}>
                      <div className={`absolute -left-[5px] top-0 w-2.5 h-2.5 rounded-full ${isDarkMode ? 'bg-zinc-600' : 'bg-zinc-300'}`}></div>
                      <div>
                         <p className={`text-xs ${theme.textSec}`}>Installation</p>
                         <p className={`text-[10px] ${theme.textSec}`}>15 Jan 2025 • by Sola Team</p>
                      </div>
                   </div>
                </div>
             </div>
           ...
           {activeTab === 'billing' && (
             <div className="space-y-6 animate-fade-in">
                <div className={`bg-gradient-to-r ${isDarkMode ? 'from-zinc-800 to-zinc-900 border-zinc-700' : 'from-zinc-100 to-white border-zinc-200'} border rounded-xl p-4 text-center`}>
                   <p className={`text-[10px] uppercase tracking-widest mb-1 ${theme.textSec}`}>{t.profile_current_plan}</p>
                   <h2 className={`text-2xl font-bold mb-1 ${theme.text}`}>Sola Enterprise</h2>
                   <p className={`text-xs ${theme.accentText}`}>Active until Dec 2026</p>
                </div>

                <div className="space-y-2">
                   <h3 className={`font-bold text-sm mb-2 ${theme.text}`}>{t.profile_invoices}</h3>
                   {[
                     {id: 'INV-2025-10', date: '01 Oct 2025', amount: 'IDR 450.000'},
                     {id: 'INV-2025-09', date: '01 Sep 2025', amount: 'IDR 450.000'},
                     {id: 'INV-2025-08', date: '01 Aug 2025', amount: 'IDR 450.000'},
                   ].map((inv, i) => (
                      <div key={i} className={`flex items-center justify-between p-3 border ${theme.border} rounded-lg ${isDarkMode ? 'bg-zinc-900 hover:bg-zinc-800' : 'bg-white hover:bg-zinc-50'} transition-colors cursor-pointer group`}>
                         <div className="flex items-center gap-3">
                            <FileText size={16} className={`${theme.textSec} group-hover:${theme.text}`}/>
                            <div>
                               <p className={`text-xs font-medium ${theme.text}`}>{inv.id}</p>
                               <p className={`text-[10px] ${theme.textSec}`}>{inv.date}</p>
                            </div>
                         </div>
                         <div className="flex items-center gap-3">
                            <span className={`text-xs ${theme.textSec}`}>{inv.amount}</span>
                            <Download size={14} className={theme.accentText}/>
                         </div>
                      </div>
                   ))}
                </div>

                <div className={`border ${theme.border} rounded-xl p-3 flex items-center gap-3 ${isDarkMode ? 'bg-zinc-900' : 'bg-white'}`}>
                   <ShieldCheck className="text-emerald-500" size={20}/>
                   <div className="flex-1">
                      <p className={`text-xs font-bold flex items-center gap-2 ${theme.text}`}>
                        SLA Agreement.pdf
                        <button onClick={() => openInfo('sla')} className="hover:text-current"><HelpCircle size={10} /></button>
                      </p>
                      <p className={`text-[10px] ${theme.textSec}`}>Signed on 15 Jan 2025</p>
                   </div>
                   <button className={`text-xs underline ${theme.accentText}`}>View</button>
                </div>
             </div>
           )}
...
// --- INFO MODAL ---
const InfoModal = ({ infoKey, t, theme, onClose }) => {
    const content = {
    ...
    return (
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-8 animate-fade-in" onClick={onClose}>
...
const NavItem = ({ icon, active, label, onClick, theme }) => (
...
const BusinessView = ({ t, theme, isDarkMode, productionVol, setProductionVol, openInfo, onExport }) => {
...
const HomeView = ({ t, theme, isDarkMode, dreamGoal, onEditGoal, onShare }) => {
...
const EnergyView = ({ t, theme, isDarkMode, openInfo }) => {
...
const NotificationsModal = ({ t, theme, notifications, onClose }) => (
  <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-6 animate-fade-in" onClick={onClose}>
